...
extension RichTextAttributeWriter {
    ...
    public func setRichTextAttributes(
        ...
        attributes.forEach { attribute, newValue in
            string.enumerateAttribute(attribute, in: range, options: .init()) {
                _, range, _ in
...
extension RichTextAttributeWriter {
    /**
     Remove a certain rich text attribute to a certain value at
     the provided range.
    ...
    public func removeRichTextAttribute(
        ...
        removeRichTextAttributes([attribute], at: range)
    }

    /**
     Remove a set of rich text attributes at the provided range.
    ...
    public func removeRichTextAttributes(
        _ attributes: [RichTextAttribute],
        at range: NSRange? = nil
    ) {
        let rangeValue = range ?? richTextRange
        let range = safeRange(for: rangeValue)
