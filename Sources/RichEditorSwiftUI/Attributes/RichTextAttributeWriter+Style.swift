...
extension NSMutableAttributedString {
    ...
    public func setRichTextStyle(
        ...
        if style == .underline {
            return setRichTextAttribute(
                .underlineStyle, to: attributeValue, at: range)
        }
        if style == .strikethrough {
            return setRichTextAttribute(
                .strikethroughStyle, to: attributeValue, at: range)
        }
        ...
        if let richTextStyle = style.richTextStyle,
            !style.isDefault && !style.isHeaderStyle
        {
            descriptor = descriptor.byTogglingStyle(richTextStyle)
        }
        let newFont: FontRepresentable? = FontRepresentable(
            descriptor: descriptor,
            size: byTogglingFontSizeFor(
                style: style, font: font, shouldAdd: newValue))
     ...
     */
    private func byTogglingFontSizeFor(
        style: RichTextSpanStyle, font: FontRepresentable, shouldAdd: Bool
    ) -> CGFloat {
        guard style.isHeaderStyle || style.isDefault else {
